...
> 面向公众号 / 博客 / 小红书等渠道的成篇图文教程。与 articles/ 的区别：
> articles 是课程学习笔记，tutorials 是独立成篇、带大量配图的长文。
> 配图统一放在 [assets/](assets/)，Markdown 与 HTML 用相对路径 `assets/xxx.png` 引用。
...
- 网站实拍截图：`site-*.png`，保留浏览器视口，注明截图时间
- 真实运行截图：`term-*.png`，终端卡片风格，必须是真实执行结果
- Colab 实机截图：`colab-*.png`，直接截取 Colab 界面，注明截图时间
- 自制图表：`diagram-*.png` / `montage-*.png`，蓝黄主色（#3776AB / #FFD43B）
- 文内引用需标注数据时点，转载要求注明仓库出处
